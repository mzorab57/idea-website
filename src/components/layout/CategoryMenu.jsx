...
export default function CategoryMenu({ open, onClose }) {
  ...
  return (
    <div
      ...
      className="absolute top-full inset-x-0 z-50  max-w-6xl mx-auto" // Added padding-top for detached look
      ref={panelRef}
    >
      {/* The Menu Box */}
      <div className="mx-auto max-w-7xl px-4 ">
        <div className="overflow-hidden rounded-3xl  border border-orange-300 bg-white shadow-2xl shadow-slate-200/60 ring-1 ring-slate-100 flex h-[350px]">
          ...
          <div className="flex-1 p-8 bg-gradient-to-br from-white via-white to-orange-50/30">
            {currentActiveId ? (
              <div className="h-full flex flex-col animate-fadeIn">
                ...
                <div className="flex items-end justify-between border-b border-orange-300 pb-5 mb-6">
                  <div>
                  ...
                  <button 
                    onClick={() => navigate(`/books?category_id=${encodeURIComponent(currentActiveId)}`)}
                ...
                <div className="grid grid-cols-4 gap-4 content-start overflow-y-auto pr-2 pb-4">
                  {(activeSubs || []).map((s, idx) => 
                    <button
                      key={s.id}
                      onClick={() => navigate(`/books?category_id=${encodeURIComponent(currentActiveId)}&subcategory_id=${encodeURIComponent(s.id)}`)}
                      className="group relative flex  items-start p-4 rounded-2xl border border-slate-100 bg-white hover:border-orange-200 hover:shadow-lg hover:shadow-orange-500/5 transition-all duration-300 hover:-translate-y-1"
                      style={{ animationDelay: `${idx * 50}ms` }}
                    >
                      

                      <div className="mb-3 h-8 w-8 ml-1 rounded-full bg-orange-50 flex items-center justify-center text-orange-500 group-hover:bg-orange-500 group-hover:text-white transition-colors duration-300">
                        <BookOpen size={16} />
                      </div>
